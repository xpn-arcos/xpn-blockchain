...
This sample provides an environment to display and capture metrics from the text-xpn in real time. It consists of a docker-compose file that starts a Prometheus and Grafana server setup configured to collect and display metrics for the test network.
...
You will need to have installed **docker-compose with version 1.29 or above** (note that this is higher than the v1.14 requirement requested for the text-xpn).

## How to use

1. Go to the text-xpn directory and run bring up the text-xpn **./network.sh up createChannel**
2. Bring up the Prometheus/Grafana network in the text-xpn/prometheus-grafana directory and run **docker-compose up -d**
...
5. Deploy a chaincode (i.e. "./network.sh deployCC -ccn basic -ccp ../asset-transfer-xpn/chaincode-go -ccl go"), start using the text-xpn and use the Grafana dashboard to analyse and assess your network performances.
